...
- Workspace crates (root `Cargo.toml`): `lsf/` (CLI), `cardinal-sdk/` (crate `hajimi-sdk`, shared types), `fswalk/`, `fs-icon/`, `namepool/`, `query-segmentation/`, `search-cache/`, `search-cancel/`, `cardinal-syntax/` (crate `hajimi-syntax`), `slab-mmap/` (mmap-backed slab for the cache), `was/` (CLI that streams FSEvents via the SDK).
