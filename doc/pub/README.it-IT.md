<div align="center">
  <img src="../../app-icon.png" alt="Icona di hajimi" width="120" height="120">
  <h1>hajimi</h1>
  <p>App di ricerca file per macOS più veloce e precisa.</p>
  <p>
    <a href="#utilizzare-hajimi">Utilizzare hajimi</a> ·
    <a href="#compilare-hajimi">Compilare hajimi</a>
  </p>
  <img src="UI.gif" alt="Anteprima dell'interfaccia di hajimi" width="720">
</div>
...
Scarica l'ultimo pacchetto `.dmg` dalle [GitHub Releases di hajimi](https://github.com/WaveF/hajimi/releases/).

### Installazione

1. Apri il file `.dmg` scaricato.
2. Trascina `hajimi.app` nella cartella `Applications`.
3. Apri hajimi da `Applications`.

La versione macOS attuale è firmata ad hoc e non è ancora stata notarizzata da Apple. Se macOS blocca l’app, aprila una volta, poi vai in **Impostazioni di Sistema → Privacy e sicurezza → Apri comunque** e conferma. Fallo solo con un DMG scaricato dalla GitHub Release ufficiale di hajimi.

In alternativa, dopo aver spostato l’app in `Applications`, puoi rimuovere dal terminale l’attributo di quarantena del download:
...
hajimi ora affianca alla classica corrispondenza per sottostringa/prefisso una sintassi compatibile con Everything:
...
- `Cmd+Shift+Space` – attiva o chiude la finestra di hajimi globalmente tramite scorciatoia rapida.
- `Cmd+,` – apre le Preferenze.
- `Esc` – nasconde la finestra di hajimi.
- `ArrowUp`/`ArrowDown` – sposta la selezione.
- `Shift+ArrowUp`/`Shift+ArrowDown` – estende la selezione.
- `Space` – Quick Look della riga selezionata senza lasciare hajimi.
...
hajimi è nato da [Cardinal](https://github.com/cardisoft/cardinal). Grazie di cuore agli autori e ai contributori originali per averci lasciato una base così solida: da qui continuiamo a fare esperimenti e migliorare.
