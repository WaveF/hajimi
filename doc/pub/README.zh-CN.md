...
请从 [hajimi GitHub Releases](https://github.com/WaveF/hajimi/releases/) 下载最新的 `.dmg` 安装包。

### 安装

1. 打开下载好的 `.dmg` 文件。
2. 将 `hajimi.app` 拖入 `Applications` 文件夹。
3. 从 `Applications` 文件夹打开 hajimi。

当前 macOS 版本使用 ad-hoc 签名，尚未经过 Apple 公证。如果 macOS 阻止打开，请先尝试打开一次，然后前往 **系统设置 → 隐私与安全性 → 仍要打开**，确认允许运行。请仅对从 hajimi 官方 GitHub Release 下载的 DMG 执行此操作。

如果需要使用命令行，先将应用移入 `Applications`，然后可以移除下载文件的隔离属性：
...
hajimi 现在在经典的子串/前缀匹配基础上叠加了 Everything 兼容语法：
...
- `Cmd+Shift+Space` – 通过全局快捷键开/关 hajimi 窗口。
- `Cmd+,` – 打开偏好设置。
- `Esc` – 隐藏 hajimi 窗口。
- `ArrowUp`/`ArrowDown` – 上下移动选中项。
- `Shift+ArrowUp`/`Shift+ArrowDown` – 扩展选中范围。
- `Space` – 不离开 hajimi 即可对当前行执行 Quick Look。
- `Cmd+O` – 打开选中的结果。
...
hajimi 是从 [Cardinal](https://github.com/cardisoft/cardinal) 这块坚实的基础上继续长出来的。感谢原项目作者和贡献者们，辛苦搭好地基，接下来我们就从这里继续折腾啦。
