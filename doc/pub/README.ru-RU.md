...
Скачайте последний пакет `.dmg` из [GitHub Releases hajimi](https://github.com/WaveF/hajimi/releases/).

### Установка

1. Откройте скачанный файл `.dmg`.
2. Перетащите `hajimi.app` в папку `Applications`.
3. Запустите hajimi из папки `Applications`.

Текущая версия для macOS подписана ad-hoc и ещё не прошла нотариальную проверку Apple. Если macOS заблокирует приложение, один раз откройте его, затем перейдите в **Системные настройки → Конфиденциальность и безопасность → Всё равно открыть** и подтвердите запуск. Используйте этот способ только для DMG, скачанного из официального GitHub Release hajimi.

В качестве альтернативы в командной строке после перемещения приложения в `Applications` можно удалить атрибут карантина загрузки:
...
hajimi вырос из [Cardinal](https://github.com/cardisoft/cardinal). Большое спасибо авторам и участникам исходного проекта за такую крепкую основу — дальше продолжаем понемногу улучшать и экспериментировать.
