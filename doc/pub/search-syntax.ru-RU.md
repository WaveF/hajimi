# Синтаксис поиска hajimi

Язык запросов hajimi намеренно близок к синтаксису Everything, но отражает то, что текущий движок действительно реализует. Эта страница — эталонная справка о том, что сегодня понимает Rust‑бэкенд.
...
- Токен без кавычек и без `/` — это **поиск по подстроке** в одном компоненте пути:
  ...
  - Он не совпадает с `/Users/demo/Projects/hajimi.md` только потому, что предок называется `demo`; используйте `demo/**`, чтобы искать потомков.
...
hajimi понимает “сегменты со слешами” внутри токена и классифицирует каждый сегмент как префиксное/суффиксное/точное/подстрочное совпадение по компонентам пути. Примеры:
...
hajimi следует приоритетам Everything:
...
> **Примечание**: аргументы фильтра должны идти сразу после двоеточия (`ext:jpg`, `parent:/Users/demo`). Запись `file: *.md` вставляет пробел, поэтому hajimi трактует это как фильтр `file:` (без аргумента), за которым следует отдельный токен `*.md`.
...
Фильтрует по тегам Finder (macOS). hajimi запрашивает теги по необходимости из метаданных файла (без кэша) и для больших наборов результатов использует `mdfind`, чтобы сузить кандидатов перед применением сопоставления тегов.
