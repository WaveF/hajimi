<div align="center">
  <img src="../../app-icon.png" alt="Іконка hajimi" width="120" height="120">
  <h1>hajimi</h1>
  <p>Найшвидший і найточніший застосунок для пошуку файлів на macOS.</p>
  <p>
    <a href="#використання-hajimi">Використання hajimi</a> ·
    <a href="#збірка-hajimi">Збірка hajimi</a>
  </p>
  <img src="UI.gif" alt="Попередній перегляд інтерфейсу hajimi" width="720">
</div>
...
2. Перетягніть `hajimi.app` до папки `Applications`.
3. Відкрийте hajimi з папки `Applications`.

Поточна версія для macOS має ad-hoc підпис і ще не пройшла нотаризацію Apple. Якщо macOS заблокує програму, відкрийте її один раз, потім перейдіть до **Системні параметри → Приватність і безпека → Відкрити все одно** та підтвердьте запуск. Робіть це лише для DMG, завантаженого з офіційного GitHub Release hajimi.

Як альтернативу в командному рядку після переміщення програми до `Applications` можна видалити атрибут карантину завантаження:
...
hajimi підтримує Everything-сумісний синтаксичний шар поверх класичного пошуку за підрядком/префіксом:
...
- `Cmd+Shift+Space` – глобально перемикає вікно hajimi гарячою клавішею швидкого запуску.
- `Cmd+,` – відкриває налаштування.
- `Esc` – приховує вікно hajimi.
- `ArrowUp`/`ArrowDown` – переміщує виділення.
- `Shift+ArrowUp`/`Shift+ArrowDown` – розширює виділення.
- `Space` – Quick Look вибраного рядка, не виходячи з hajimi.
...
hajimi виріс із [Cardinal](https://github.com/cardisoft/cardinal). Велике спасибі авторам і учасникам оригінального проєкту за таку міцну основу — далі продовжуємо потроху вдосконалювати й експериментувати.
